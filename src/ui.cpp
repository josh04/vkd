...
#undef min
#endif
//static azure::TimePoint       g_Time = azure::TimePoint::min();
...
imguiDrawer::imguiDrawer(unsigned int width, unsigned int height) : _width(width), _height(height) {
...
void ImGui_ImplGlfwGL3_CreateFontsTexture(ImFont** font)
{
    ImGuiIO& io = ImGui::GetIO();

    //auto path = std::string("resources/cabin-bold.ttf");

    //auto wi = azure::Engine::GetWindowInterface();
    //auto&& vec = wi->readAllVector(path);

    //auto def = io.Fonts->AddFontDefault();

    //ImFontConfig config;
    //config.FontDataOwnedByAtlas = false;
    //*font = io.Fonts->AddFontFromMemoryTTF(vec.data(), vec.size(), 48.0f, &config);
...
bool ImGui_ImplGlfwGL3_CreateDeviceObjects(ImFont** font)
{
    // Backup GL state
...
#undef OFFSETOF
    ...
    ImGui_ImplGlfwGL3_CreateFontsTexture(font);
...
void imguiDrawer::preRender() {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        /*if (mouseButtons[i] == true) {
         std::cout << "Button: " << i << " x: " << m_x << " y: " << m_y << std::endl;
         }*/
