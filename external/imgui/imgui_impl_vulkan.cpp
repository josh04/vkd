...
namespace {
    std::vector<VkDescriptorSet> desc_sets_to_delete;
}

void ImGui_ImplVulkan_RemoveTexture(ImTextureID tex) {
    desc_sets_to_delete.push_back((VkDescriptorSet)tex);
}

void ImGui_ImplVulkan_CleanupDescriptors() {
    if (desc_sets_to_delete.size()) {
        VkResult err;
        ImGui_ImplVulkan_InitInfo* v = &g_VulkanInitInfo;
        err = vkFreeDescriptorSets(v->Device, v->DescriptorPool, desc_sets_to_delete.size(), desc_sets_to_delete.data());
        check_vk_result(err);
        desc_sets_to_delete.clear();
    }
}
